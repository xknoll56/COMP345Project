...
Map* ConquestFileReader::ReadFile(const std::string path)
{
	std::string line;
	std::ifstream myfile(path);
	DataType dataType = DataType::Nil;
	if (myfile.is_open()) {
		while (getline(myfile, line)) {
			// A regular expression term is used to determine if a new header is found
			// (of the form [XXXXXX]).
			std::regex title("\\[[a-z_A-Z]+\\]");
			std::smatch m;
			if (regex_search(line, m, title)) {
				// If the regular expression is found, it should be one of the
				// following, and the corresponding validity data is set
				if (m.str().compare("[Continents]") == 0) {
					dataType = DataType::Continents;
				}
	...
	map = new Map(continentsData.size(), territoriesData.size());
	//The data is now extracted and it can be processed
	ProcessContinents();
	ProcessTerritories();
	ProcessBorders();
	return map;
...
std::vector<std::string> ConquestFileReader::Split(const std::string line) const {
	// Create a vector to store the strings
	std::vector<std::string> splitStrings;
	// Create indices variables for the start indicy and end indicy of the split
	// string
...
void ConquestFileReader::PreprocessContinents(const std::string line, DataType* const dataType) {
	if (line.compare("") != 0) {
		continentsData.push_back(line);
	}

}
void ConquestFileReader::PreprocessTerritories(const std::string line, DataType* const dataType) {
	if (line.compare("") != 0) {
		territoriesData.push_back(Split(line));
	}

}

Territory* ConquestFileReader::GetTerritoryByName(std::string name) {
	Territory* territory = nullptr;
	for (Territory* _territory : *map->GetTerritories()) {
		if (_territory->GetName()->compare(name) == 0) {
...
Continent* ConquestFileReader::GetContinentByName(std::string name) {
	Continent* continent = nullptr;
	for (Continent* _continent : map->GetContinents()) {
		if (_continent->GetName()->compare(name) == 0) {
...
void ConquestFileReader::ProcessContinents() {
	for(std::string line: continentsData){
		std::string name = "";
		int bonus;
		int i;
		...
		bonus = std::stoi(line.substr(i + 1));
		map->CreateContinent(name, bonus);
	}
}
void ConquestFileReader::ProcessTerritories() {
	for (std::vector<std::string> data : territoriesData) {
		Continent* continent = GetContinentByName(data[3]);
		map->CreateTerritory(data[0], continent);
	}
}
void ConquestFileReader::ProcessBorders() {
	for (std::vector<std::string> data : territoriesData) {
		Territory* territory = GetTerritoryByName(data[0]);
		for (int i = 4; i < data.size(); ++i) {
			Territory* toAdd = GetTerritoryByName(data[i]);
			territory->AddNeigbor(toAdd);
		}
	}
}
...
Map* ConquestFileReaderAdapter::GenerateMap(const std::string filePath){
	return conquestFileReader->ReadFile(filePath);
}
